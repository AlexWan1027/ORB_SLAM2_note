...
/**
 * @brief 局部建图线程中的入口，即该线程的主函数
 *
 * 跟踪线程将新产生的关键帧传给局部建图线程，局部线程会将该新的关键帧插入
 * 
 * 可以通过深度值产生一些新的MapPoints
 * @see VI local mapping
 */
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        // 设置接收关键帧标志位为false，即告诉Tracking，LocalMapping正处于繁忙状态，
        // LocalMapping线程处理的关键帧都是Tracking线程发过的
        // 在LocalMapping线程还没有处理完关键帧之前Tracking线程最好不要发送太快
        SetAcceptKeyFrames(false);

        // Check if there are keyframes in the queue
	// 检测保存新来关键帧的列表mlNewKeyFrames是否为空
	// 如果跟踪线程产生了新的关键帧，那么就处理该关键帧
        if(CheckNewKeyFrames())
        {
            // BoW conversion and insertion in Map
	    // 论文《ORB_SLAM:a versatile and ....》 VI-A：keyframe insertion
            // 计算关键帧特征点的BoW映射，将关键帧插入地图
	    // 取出一帧关键帧，队列中会少一帧关键帧
            ProcessNewKeyFrame();

            // Check recent MapPoints
	    // 论文《ORB_SLAM:a versatile and ....》 VI-B recent map points culling
            // 剔除ProcessNewKeyFrame函数中引入的不合格MapPoints
            MapPointCulling();

            // Triangulate new MapPoints
	    // VI-C new map points creation
            // 当前新关键帧中还未匹配的特征与相邻关键帧未匹配的特征通过三角化得到新的MapPoints
            CreateNewMapPoints();
	    ...
	    // 已经处理完队列中的最后的一个关键帧
	        ...
	        // 在相邻帧中去搜索更多特征的匹配 
	        // 误差的存在可能当前帧图像对应的地图点在相邻帧有对应的特征
	        // 而这个特征已经生成了地图点，这时候需将这两个地图点融合为一个地图点
	        // 检查并融合当前关键帧与相邻帧（两级相邻）重复的MapPoints
                SearchInNeighbors();
            }

            // 中断局部光束平差法的标志位置为false
            mbAbortBA = false;
...
/**
 * @brief 处理列表中的从跟踪线程新进来的关键帧
 * 
 * - 计算关键帧的Bow表示，主要用于加速三角化新的MapPoints（数据关联）
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // 将在跟踪局部地图过程中新匹配上的MapPoints和当前关键帧绑定
 ...
 * 
 * -地图点需通过严格的测试才能放到地图中
 * @see VI-C new map points creation
 */
void LocalMapping::CreateNewMapPoints()
{
    // Retrieve neighbor keyframes in covisibility graph
    // 用于在共视图中检索得到相邻关键帧共视最多的关键帧数量
    int nn = 10;
    if(mbMonocular)
        nn=20;
   // 在共视图中找到与当前关键帧共视程度最高的前nn帧相邻帧vpNeighKFs
	...
	// Ow2为第i帧相邻关键帧的世界坐标系的坐标
        cv::Mat Ow2 = pKF2->GetCameraCenter();
	// 基线向量并可以通过对其求摸得到基线的长度
	...
	// 通过极线约束限制匹配时的搜索范围，进行特征点匹配，返回匹配的数量
	...
	// 每对匹配通过三角化生成3D点,he Triangulate函数差不多
        const int nmatches = vMatchedIndices.size();
        for(int ikp=0; ikp<nmatches; ikp++)
        {
	    // 当前匹配对在当前关键帧和相邻关键帧中的索引
            const int &idx1 = vMatchedIndices[ikp].first;
            const int &idx2 = vMatchedIndices[ikp].second;

	    // 当前关键帧的关键点
            const cv::KeyPoint &kp1 = mpCurrentKeyFrame->mvKeysUn[idx1];
	    // mvuRight中存放着双目的深度值，如果不是双目，其值将为-1,则bStereo1为false
...
void LocalMapping::SearchInNeighbors()
{
    ...
    // 
    // vpFuseCandidates用于存储一级邻接和二级邻接关键帧所有MapPoints的集合
